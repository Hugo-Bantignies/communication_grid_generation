...
def compute_word_similarities(voc,model):
    ...
    for i in tqdm(range(len(voc)),desc = "Similarities computation ",unit = "word"):
        ...
        for wj in voc:
            ...
            #Compute the dissimilarity between the two words
...
def page_similarity_cost(page,sim_matrix,synonyms_refs = None,missmatch_mode = False,stopwords = [],encountered_words = []):
    ...
    for wi in words:
        ...
        if(wi not in encountered_words):
            enc_words.append(wi)
        ...
        for wj in words:
            ...
            #If synonym pictograms
            if(synonyms_refs != None):
                if(wa in synonyms_refs):
                    wa = synonyms_refs[wa]
            if(synonyms_refs != None):
                if(wb in synonyms_refs):
                    wb = synonyms_refs[wb]

            word_score += sim_matrix[wa][wb]
    ...
    if(len(words) == 0):
        wc = 1
    return cost/(wc*wc),enc_words

def grid_similarity_cost(grid,sim_matrix,synonyms_file = None,missmatch_mode = False,stopwords = []):
    '''Function to compute the similarity cost of an entire grid'''

    cost = 0

    sim_mat = load_similarity_matrix(sim_matrix)
    synonyms_refs = None
    encountered_words = []
    ...
    for page in grid.pages.values():
        ...
        c,ecw = page_similarity_cost(page,sim_mat,synonyms_refs,missmatch_mode,stopwords,encountered_words)
        cost += c
        encountered_words = encountered_words + ecw

    return cost

def grid_cost(grid,input_corpus,sim_matrix,similarity_coefficient = 0.5,synonyms_file = None, missmatch_mode = False, stopwords = []):
    ...
    if(similarity_coefficient == 0):
        dist,mismatches,mismatch_list,stats =  grid_distance_cost(grid,input_corpus,synonyms_file,missmatch_mode,stopwords)
        return dist, mismatches, mismatch_list, stats

    #If the cost is only depending on the similarity
    elif(similarity_coefficient == 1):
        sim =  grid_similarity_cost(grid,sim_mat,synonyms_file,missmatch_mode,stopwords)
        return math.log10(sim),0,[],[]
